...
- 依赖的第三方插件代码 `1423933177`它提供了核心的卡片单向链接功能,还有`564851917(可选)` ,它提供了层级标签功能,这不是必须的,但有更好.
...
### 0.3
- 新增
    - 每次建立连接都会给整个`input.json`中存在的卡片生成名字为 `hjp-bilink::YYYYMMDDHHMMSS` 的tag,这样方便你从全局角度查看同一连接的全部成员.
    - 推荐安装依赖的层级标签插件`564851917` 
### 0.2.2
- 优化
