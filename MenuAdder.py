...
def func_menuAddBrowserInsert(menu: QMenu = None, browser: Browser = None, need: tuple = ("none",)):
    """browser插入类函数集合"""
...
def func_menuAddLink(menu: QMenu = None, need: tuple = ("none",)):
    """用来给连接类型的函数加按钮"""
...
def func_menuAddClearOpen(menu: QMenu = None, need: tuple = ("none",)):
    """用来给清除和打开input功能加按钮"""
    prefix = "" if "prefix" not in need else consolerName
...
def func_menuAddBaseMenu(menu: QMenu = None):
...
def func_menuAddSingleInsert(menu: QMenu = None, card_id: str = "", desc: str = "", need: tuple = ("none",)):
    """用来添加常规插入按钮组"""
...
def func_add_webviewcontextmenu(view: AnkiWebView, menu: QMenu):
    ...
    if cid != "0":
        p = Params(parent=view, menu=menu, need=("link", "insert", "clear/open", "prefix",))
