...
from ..clipper2.lib.Model import Entity
from . import funcs, wrappers, clipper_imports
from . import all_objs
from .SrcAdmin import SrcAdmin

print, _ = clipper_imports.funcs.logger(__name__)


def func_open_clipper(pairs_li: "list[Pair]" = None, clipboxlist=None, **kwargs):
    if not isinstance(all_objs.mw_win_clipper, Clipper):
        entity = Entity()
        all_objs.mw_win_clipper = Clipper(entity)
...
def DialogSingleCheck(Dialog):
    """单一窗口的打开"""
    # consoler_Name = SrcAdmin.start().consoler_name
...
def func_webviewOpenClipper(**kwargs):
    param = Params(**kwargs)
    if param.pair.desc == "":
        param.pair.desc = funcs.desc_extract(param.pair.card_id)
    func_open_clipper(pairs_li=[param.pair])
