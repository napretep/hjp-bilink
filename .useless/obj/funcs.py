...
def clipbox_insert_cardField_suite(clipuuid, timestamp=None):
    """用于插入clipbox到指定的卡片字段"""
    DB = clipper_imports.objs.SrcAdmin.DB
    DB.go(DB.table_clipbox)
    clipbox_ = DB.select(uuid=clipuuid).return_all().zip_up()[0]
    clipbox = clipper_imports.objs.ClipboxRecord(**clipbox_)
    DB.end()
    ...
    for card_id in card_id_li:
        ...
        pdfinfo = clipper_imports.objs.PDFinfoRecord(**pdfinfo_)
        ...
        doc: "clipper_imports.fitz.Document" = clipper_imports.fitz.open(pdfinfo.pdf_path)
        toc = doc.get_toc()
        if len(toc) > 0:
            # 读取缓存的书签
            jsonfilename = os.path.join(tempfile.gettempdir(),
                                        clipper_imports.funcs.uuid_hash_make(pdfinfo.pdf_path) + ".json")
...
def HTML_clipbox_PDF_info_dict_read(root):
    ...
    for record in record_li:
        pdfname = DB.go(DB.table_pdfinfo).select(uuid=record["pdfuuid"]).return_all().zip_up()[
            0].to_pdfinfo_data().pdf_path
        if record["pdfuuid"] not in PDF_info_dict:
            PDF_info_dict[record["pdfuuid"]] = {"pagenum": set(),
                                                "pdfname": pdfname}  # 只提取页码, 大小重新再设定.偏移量也重新设定.
