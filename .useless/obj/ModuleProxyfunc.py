...
def on_anki_field_insert_handle1(self, clipboxlist: "list"):
    ...
    for clipbox_ in clipboxlist:  # must fetch from DB
        clipbox = DB.go(DB.table_clipbox).select(uuid=clipbox_["uuid"]).return_all().zip_up()[0]
        count += 1
        ...
        for card_id in card_id_li:
            pdfname = os.path.basename(
                DB.go(DB.table_pdfinfo).select(uuid=clipbox.to_clipbox_data().pdfuuid).return_all().zip_up()[
...
anki_card_create_handle = {
    e.defaultType.clipboxNeed: on_anki_create_card_handle1
}

e = clipper_imports.events.AnkiFieldInsertEvent
anki_field_insert_handle = {
    e.defaultType.clipboxNeed: on_anki_field_insert_handle1
}

e = clipper_imports.events.AnkiFileCreateEvent
anki_file_create_handle = {
    e.defaultType.ClipperCreatePNG: anki_file_create_handle1
}
