...
class PDFPrevDialog(QDialog):
    ...
    class RightSideBakclink(QDialog):
        ...
        def init_UI(self):
            ...
            self.model.setHorizontalHeaderLabels(["card_id"])
        ...
        def init_data(self):
            ...
            DB = clipper_imports.objs.SrcAdmin.DB
            result = DB.go(DB.table_clipbox).select(pdfuuid=pdfuuid, pagenum=page).return_all(callback=None).zip_up()
            ...
            for card_id in list(all_card_li):
                card = Item(self, Item.type.card, card_id + "_" + funcs.desc_extract(card_id))

                self.model.appendRow([card])
                card_info = clipper_imports.funcs.card__pdf_page_clipbox_info__collect(card_id)
        ...
        class Delegate(QStyledItemDelegate):
            ...
            def paint(self, painter: QtGui.QPainter, option: 'QStyleOptionViewItem', index: QtCore.QModelIndex) -> None:
                ...
                if index.data(Qt.UserRole) == item.type.pdfuuid:
                    ...
                    pdfname = clipper_imports.funcs.str_shorten(os.path.basename(pdfinfo.pdf_path))
                    ...
                    pdfinfo = DB.go(DB.table_pdfinfo).select(uuid=clip.pdfuuid).return_all().zip_up().to_pdfinfo_data()[
                        0]
                    pixmap_ = clipper_imports.funcs.png_pdf_clip(pdfinfo.pdf_path, clip.pagenum, rect1=clip.__dict__,
                                                                 ratio=clip.ratio)
                    pixmap = clipper_imports.funcs.Qpixmap_from_fitzpixmap(pixmap_)
            ...
            def sizeHint(self, option: 'QStyleOptionViewItem', index: QtCore.QModelIndex) -> QtCore.QSize:
                ...
                if index.data(Qt.UserRole) == item.type.clipuuid:
                    if index.data(self.myRole):
                        ...
                        self.superior.resize(max(6 * t + w, s.width()), s.height())
    ...
    class CenterView(QWidget):
        ...
        class Clipbox(QGraphicsRectItem):
            ...
            def __init__(self, parent: "PDFPrevDialog.CenterView.Page" = None, rect: "QRectF" = None, pagenum=None):
                super().__init__(parent)
                self.uuid = clipper_imports.funcs.clipbox_uuid_random_unique()
                self.atpage = parent
                self.pagenum = pagenum
                self.fromrect = rect
            ...
            class Close:
                def __init__(self, superior: "PDFPrevDialog.CenterView.Clipbox"):
                    ...
                    self.widget.setIcon(QIcon(clipper_imports.objs.SrcAdmin.imgDir.close))
            ...
            def run(self):
                ...
                for card_desc, clipboxli in data.items():
                    ...
                    for clipbox in clipboxli:
                        ...
                        funcs.clipbox_insert_cardField_suite(clipbox["uuid"], timestamp=timestamp)
